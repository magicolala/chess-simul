  ...
  template: `
    <div class="h-full max-w-[1920px] mx-auto p-4 flex flex-col lg:flex-row gap-4 lg:gap-0 font-sans bg-gray-100 dark:bg-[#0a0a0a]">
        ...
        <div class="ui-card w-full lg:w-96 flex flex-col h-[60vh] lg:h-auto">
            ...
            <!-- Engine Output -->
            <div class="p-4 bg-gray-50 dark:bg-[#121212] border-b-2 border-[#1D1C1C] dark:border-white min-h-[100px] flex flex-col justify-center">
                @if (isAnalyzing()) {
                    ...
                    <div class="flex justify-between items-start">
                        <div>
                            ...
                            <p class="text-2xl font-black font-mono text-[#1D1C1C] dark:text-white">{{ evalText() }}</p>
                        </div>
                    ...
                    @if (engineData()?.classification) {
                        <div class="mt-2 text-xs font-bold uppercase px-2 py-1 inline-block border-2"
                             [class.border-green-500]="engineData()?.classification === 'best' || engineData()?.classification === 'brilliant'"
                             [class.text-green-600]="engineData()?.classification === 'best' || engineData()?.classification === 'brilliant'"
                             [class.border-red-500]="engineData()?.classification === 'blunder' || engineData()?.classification === 'mistake'"
                             [class.text-red-600]="engineData()?.classification === 'blunder' || engineData()?.classification === 'mistake'">
                            {{ engineData()?.classification }}
                        </div>
                    }
                } @else {
                    <p class="text-xs text-gray-400 text-center italic">Faites un coup pour lancer l'analyse.</p>
                }
            </div>
                ...
                @if (tab() === 'moves') {
                    <div class="grid grid-cols-2 text-sm font-mono font-bold">
                        @for (movePair of moveList(); track $index) {
                            <div class="p-2 border-b border-gray-100 dark:border-gray-800 flex items-center bg-gray-50 dark:bg-[#121212] text-gray-500 border-r dark:border-gray-700 justify-center">
                                {{ $index + 1 }}.
                            </div>
