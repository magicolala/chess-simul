...
@Component({
  ...
  template: `
    <div class="max-w-4xl mx-auto p-4 md:p-8 font-sans animate-in fade-in slide-in-from-bottom-4 duration-500">
        
        <div class="text-center mb-12">
            <h2 class="text-5xl font-black font-display text-[#1D1C1C] dark:text-white uppercase tracking-tighter mb-4">Jouer contre un ami</h2>
            <p class="text-gray-500 dark:text-gray-400 font-medium text-lg">Invitez un joueur pour un défi physique ou partagez le lien.</p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div class="bg-white dark:bg-[#1a1a1a] border-2 border-[#1D1C1C] dark:border-white wero-shadow p-8 flex flex-col items-center">
                ...
                <h3 class="text-2xl font-black font-display text-[#1D1C1C] dark:text-white uppercase mb-6">Créer une partie</h3>
                ...
                <div class="w-full space-y-6 mb-8">
                    ...
                    <div class="flex justify-center space-x-4">
                        <button (click)="selectedColor.set('w')" class="w-12 h-12 rounded-full border-2 border-[#1D1C1C] flex items-center justify-center hover:bg-gray-100" [class.ring-4]="selectedColor() === 'w'" [class.ring-[#7AF7F7]]="selectedColor() === 'w'">
                            <div class="w-6 h-6 bg-white border border-gray-300 rounded-full"></div>
                        </button>
                        <button (click)="selectedColor.set('random')" class="w-12 h-12 rounded-full border-2 border-[#1D1C1C] flex items-center justify-center bg-gradient-to-r from-white to-[#1D1C1C] hover:opacity-80" [class.ring-4]="selectedColor() === 'random'" [class.ring-[#7AF7F7]]="selectedColor() === 'random'">
                            <span class="mix-blend-difference text-white font-bold font-display">?</span>
                        </button>
                        <button (click)="selectedColor.set('b')" class="w-12 h-12 rounded-full border-2 border-[#1D1C1C] bg-[#1D1C1C] flex items-center justify-center hover:bg-gray-800" [class.ring-4]="selectedColor() === 'b'" [class.ring-[#7AF7F7]]="selectedColor() === 'b'">
