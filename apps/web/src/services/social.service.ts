...
export class SocialService {
  ...
  friends = signal<Friend[]>([
    { id: 'alice', name: 'Alice_Gambit', avatar: 'https://api.dicebear.com/7.x/notionists/svg?seed=Alice', status: 'ingame', activity: 'Blitz 3+2', elo: 1450 },
    { id: 'bob', name: 'BobbyFisherPrice', avatar: 'https://api.dicebear.com/7.x/notionists/svg?seed=Bob', status: 'online', elo: 1200 },
    { id: 'charlie', name: 'CheckMate_Pro', avatar: 'https://api.dicebear.com/7.x/notionists/svg?seed=Charlie', status: 'offline', elo: 1890 },
    { id: 'dave', name: 'DaveTheRook', avatar: 'https://api.dicebear.com/7.x/notionists/svg?seed=Dave', status: 'online', elo: 800 },
  ]);

  friendRequests = signal<{id: string, name: string, avatar: string}[]>([
      { id: 'eve', name: 'Eve_Opening', avatar: 'https://api.dicebear.com/7.x/notionists/svg?seed=Eve' }
  ]);
  ...
  constructor() {
      ...
      const m = new Map();
      m.set('alice', [
          { senderId: 'alice', text: 'Bien joué pour la dernière partie !', timestamp: Date.now() - 100000 },
  ...
  loadProfile(userId: string) {
      ...
      const profile: UserProfile = {
          id: userId,
          name: isMe ? this.auth.currentUser()!.name : (this.friends().find(f => f.id === userId)?.name || 'Unknown Player'),
          avatar: isMe ? this.auth.currentUser()!.avatar : (this.friends().find(f => f.id === userId)?.avatar || `https://api.dicebear.com/7.x/notionists/svg?seed=${userId}`),
          ...
          badges: [
              { id: '1', icon: '🔥', name: 'On Fire', description: 'Gagné 5 parties de suite', unlockedAt: Date.now() },
              { id: '2', icon: '🛡️', name: 'Mur de Brique', description: 'Nulle contre un joueur +200 ELO', unlockedAt: Date.now() },
              { id: '3', icon: '🎓', name: 'Apprenti', description: 'Complété le tutoriel', unlockedAt: Date.now() }
          ],
  ...
  sendMessage(friendId: string, text: string) {
      ...
      setTimeout(() => {
          ...
          updatedMap.set(friendId, [...updatedChat, {
              senderId: friendId,
