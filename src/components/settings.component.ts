  ...
  template: `
    <div class="flex flex-col h-full bg-white dark:bg-[#1a1a1a] text-[#1D1C1C] dark:text-white font-['Inter'] transition-colors duration-300">
        ...
        <div class="flex-1 flex overflow-hidden">
            ...
            <!-- Sidebar -->
            <aside class="w-64 border-r-2 border-[#1D1C1C] dark:border-white bg-gray-50 dark:bg-[#121212] flex-shrink-0 overflow-y-auto hidden md:block">
                <nav class="p-4 space-y-2">
                    <button (click)="currentTab.set('profile')" [class.active-tab]="currentTab() === 'profile'" class="tab-btn">👤 Profil</button>
                    <button (click)="currentTab.set('security')" [class.active-tab]="currentTab() === 'security'" class="tab-btn">🔒 Sécurité</button>
                    <button (click)="currentTab.set('appearance')" [class.active-tab]="currentTab() === 'appearance'" class="tab-btn">🎨 Apparence</button>
                    <button (click)="currentTab.set('game')" [class.active-tab]="currentTab() === 'game'" class="tab-btn">♟ Jeu</button>
                    <button (click)="currentTab.set('notifications')" [class.active-tab]="currentTab() === 'notifications'" class="tab-btn">🔔 Notifications</button>
                    <button (click)="currentTab.set('privacy')" [class.active-tab]="currentTab() === 'privacy'" class="tab-btn">👁 Confidentialité</button>
                    <button (click)="currentTab.set('premium')" [class.active-tab]="currentTab() === 'premium'" class="tab-btn text-[#7AF7F7] bg-[#1D1C1C]">💎 Premium</button>
                </nav>
            </aside>
            ...
            <div class="flex-1 overflow-y-auto p-6 md:p-10 space-y-8 bg-white dark:bg-[#1a1a1a]">
                ...
                <div class="md:hidden mb-6">
                    ...
                    <select [ngModel]="currentTab()" (ngModelChange)="currentTab.set($event)" class="w-full p-2 border-2 border-[#1D1C1C] dark:border-white bg-white dark:bg-black font-bold mt-1">
                    ...
                    <section class="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                        ...
                        <div class="flex flex-col md:flex-row gap-6">
                            <div class="flex flex-col items-center gap-2">
                                <img [src]="auth.currentUser()?.avatar" class="w-32 h-32 border-4 border-[#1D1C1C] dark:border-white bg-gray-100">
                                <div class="flex gap-2">
                                    <input [formControl]="avatarSeed" class="w-32 px-2 py-1 text-xs border-2 border-[#1D1C1C] dark:border-white bg-transparent" placeholder="Seed...">
                                    <button (click)="randomizeAvatar()" class="px-2 py-1 bg-[#1D1C1C] text-white text-xs font-bold">🎲</button>
                                </div>
                            </div>
                            <div class="flex-1 space-y-4">
                                <div>
                                    <label class="input-label">Nom d'utilisateur</label>
                                    <input [value]="auth.currentUser()?.name" disabled class="input-field opacity-50 cursor-not-allowed">
                                </div>
                                <div>
                                    <label class="input-label">Biographie</label>
                                    <textarea [formControl]="bioControl" rows="4" class="input-field" placeholder="Parlez de votre style de jeu..."></textarea>
                                </div>
                            </div>
                        </div>
                    </section>
                ...
                @if (currentTab() === 'security') {
                    <section class="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                        ...
                        <div class="p-4 border-2 border-red-200 bg-red-50 dark:bg-red-900/10 dark:border-red-800">
                            <h4 class="font-bold text-red-600 dark:text-red-400 uppercase text-xs mb-2">Zone Sensible</h4>
                            <p class="text-sm">Assurez-vous d'utiliser un mot de passe fort.</p>
                        </div>

                        <div>
                            <label class="input-label">Email</label>
                            <input [value]="auth.currentUser()?.email" disabled class="input-field opacity-50">
                            <p class="text-xs text-green-600 font-bold mt-1" *ngIf="auth.currentUser()?.emailVerified">✓ Email vérifié</p>
                        </div>
                        ...
                        <div class="flex items-center justify-between p-4 border-2 border-[#1D1C1C] dark:border-white">
                            <div>
                            ...
                            <div class="relative inline-block w-12 h-6 transition duration-200 ease-in-out">
                                <input type="checkbox" [checked]="auth.currentUser()?.twoFactorEnabled" (change)="toggle2FA()" class="absolute opacity-0 w-full h-full cursor-pointer z-10">
                                <span class="block w-full h-full bg-gray-300 dark:bg-gray-700 rounded-full shadow-inner border border-gray-400"></span>
                                <span class="absolute block w-4 h-4 bg-white rounded-full shadow inset-y-1 left-1 transition-transform duration-200"
                                      [class.translate-x-6]="auth.currentUser()?.twoFactorEnabled"
                                      [class.bg-[#1D1C1C]]="auth.currentUser()?.twoFactorEnabled"></span>
                            </div>
                        </div>
                    </section>
                }
                ...
                @if (currentTab() === 'appearance') {
                    <section class="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                                @for (theme of themes; track theme.id) {
                                    <div (click)="tempThemeId.set(theme.id)" 
                                        ...
                                        <div class="p-2 text-xs font-bold uppercase text-center bg-white dark:bg-black text-[#1D1C1C] dark:text-white">
                                            {{ theme.name }}
                                        </div>
                                    </div>
                                }
                        ...
                        <!-- Dark Mode -->
                        <div class="flex items-center justify-between p-4 bg-gray-100 dark:bg-gray-800 border-2 border-transparent">
                            <span class="font-bold uppercase">Mode Sombre</span>
                            <button (click)="toggleDarkMode()" class="text-2xl">{{ tempDarkMode() ? '🌙' : '☀️' }}</button>
                        </div>
                    </section>
                }
                    ...
                    <section class="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                        ...
                        <div class="space-y-4">
                            <div class="setting-row">
                                ...
                                <input type="checkbox" [checked]="prefs.gameSettings().autoQueen" (change)="updateGameSetting('autoQueen', $event)" class="toggle-checkbox">
                            </div>
                            <div class="setting-row">
                                <span>Confirmer l'abandon</span>
                                <input type="checkbox" [checked]="prefs.gameSettings().confirmResign" (change)="updateGameSetting('confirmResign', $event)" class="toggle-checkbox">
                            </div>
                            <div class="setting-row">
                                <span>Sons du jeu</span>
                                <input type="checkbox" [checked]="prefs.gameSettings().soundEnabled" (change)="updateGameSetting('soundEnabled', $event)" class="toggle-checkbox">
                            </div>
                            
                            <div>
                                <label class="input-label">Méthode de saisie</label>
                                <div class="flex space-x-2 mt-2">
                                    <button (click)="updateInputMethod('drag')" [class.selected-pill]="prefs.gameSettings().inputMethod === 'drag'" class="pill-btn">Glisser</button>
                                    <button (click)="updateInputMethod('click')" [class.selected-pill]="prefs.gameSettings().inputMethod === 'click'" class="pill-btn">Cliquer</button>
                                    <button (click)="updateInputMethod('both')" [class.selected-pill]="prefs.gameSettings().inputMethod === 'both'" class="pill-btn">Les deux</button>
                                </div>
                            </div>
                        </div>
                    </section>
                    ...
                    <section class="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                        ...
                        <div class="space-y-4 bg-white dark:bg-[#1a1a1a] p-1">
                            <div class="setting-row">
                                ...
                                <input type="checkbox" [checked]="prefs.notifications().push" (change)="updateNotif('push', $event)" class="toggle-checkbox">
                            </div>
                            <div class="setting-row">
                                <span>Emails (Récap, Sécurité)</span>
                                <input type="checkbox" [checked]="prefs.notifications().email" (change)="updateNotif('email', $event)" class="toggle-checkbox">
                            </div>
                            <div class="setting-row">
                                <span>Son de notification (Tournoi, Message)</span>
                                <input type="checkbox" [checked]="prefs.notifications().sound" (change)="updateNotif('sound', $event)" class="toggle-checkbox">
                            </div>
                        </div>
                    </section>
                }
                ...
                @if (currentTab() === 'privacy') {
                    <section class="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                        ...
                        <div class="setting-row">
                            ...
                            <input type="checkbox" [checked]="prefs.privacy().publicProfile" (change)="updatePrivacy('publicProfile', $event)" class="toggle-checkbox">
                        </div>

                        <div>
                            <label class="input-label">Qui peut vous défier ?</label>
                            <select [value]="prefs.privacy().allowChallenges" (change)="updateChallengePrivacy($event)" class="input-field mt-2">
                        ...
                        <div class="pt-8 mt-8 border-t border-red-200">
                            ...
                            <button class="text-red-600 border-2 border-red-600 px-4 py-2 font-bold text-xs uppercase hover:bg-red-50">Supprimer mon compte</button>
                        </div>
                    </section>
                }
                ...
                @if (currentTab() === 'premium') {
                    <section class="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300 text-center">
                        <div class="w-20 h-20 bg-[#FFF48D] mx-auto flex items-center justify-center text-4xl border-2 border-[#1D1C1C] mb-4">💎</div>
                        <h3 class="text-3xl font-black font-display uppercase">Chess Master Premium</h3>
                        
                        @if (auth.currentUser()?.isPremium) {
                            <div class="bg-green-100 text-green-800 p-4 border-2 border-green-500 inline-block font-bold">
                                Vous êtes membre Premium ! Merci de votre soutien.
                            </div>
                        } @else {
                            <p class="text-gray-600 dark:text-gray-300 max-w-md mx-auto">
                                Débloquez les analyses illimitées, les thèmes exclusifs et supprimez les (futures) publicités.
                            </p>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-lg mx-auto mt-8">
                                ...
                                <div class="border-2 border-[#1D1C1C] bg-[#1D1C1C] text-white p-4 relative overflow-hidden cursor-pointer hover:opacity-90" (click)="upgrade()">
                                    <div class="absolute top-0 right-0 bg-[#FFF48D] text-[#1D1C1C] text-[10px] font-bold px-2">BEST</div>
                                    <h4 class="font-black text-xl">Annuel</h4>
                                    <p class="text-2xl my-2">39.99€</p>
                                    <p class="text-xs opacity-70">2 mois offerts</p>
                                </div>
                            </div>
                            <button (click)="upgrade()" [disabled]="auth.isLoading()" class="mt-8 px-8 py-4 bg-[#7AF7F7] text-[#1D1C1C] font-black uppercase text-xl border-2 border-[#1D1C1C] wero-shadow hover:bg-[#FFF48D] transition-colors">
        ...
        <!-- Footer -->
        <div class="p-6 border-t-2 border-[#1D1C1C] dark:border-white bg-gray-50 dark:bg-[#121212] flex justify-end space-x-4 z-10">
             <button (click)="close.emit()" class="px-6 py-3 bg-transparent border-2 border-transparent text-[#1D1C1C] dark:text-white font-bold uppercase hover:bg-gray-200 dark:hover:bg-gray-800 transition-colors text-sm">
                 Fermer
             </button>
...
export class SettingsComponent implements OnInit {
  ...
  updateNotif(key: keyof import('../services/preferences.service').NotificationSettings, event: any) {
