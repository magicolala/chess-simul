...
export interface MemoryGameState {
    ...
    targetChess: Chess; // Full game state to check moves against
    currentMoveIndex: number; // 0-indexed, corresponds to half-moves
    lives: number;
    status: 'start' | 'memorizing' | 'playing' | 'won' | 'lost' | 'duel_intermission' | 'duel_result';
    feedbackMessage: string;
    history: string[]; // FEN history for display
    timer: number; // elapsed in ms
...
39. Kc1 Ne2+ 40. Kb1 Nc3+ 41. Kc1 Rc2# 0-1`
    ...
    skipMemorization() {
        ...
        const chess = new Chess(); // Reset board logic
        // We reuse the same targetChess, but we reset the 'playing' board to empty start
    ...
    startDuelRound2() {
        const s = this.state();
        if (!s.duelState || !s.currentGame) return;
    ...
    makeMove(from: string, to: string, promotion: string = 'q'): boolean {
        ...
        if (currentState.status !== 'playing') return false;
    ...
    private endGame(result: 'won' | 'lost' | 'duel_result') {
        ...
        if (result === 'won') msg = 'Félicitations ! Vous avez une excellente mémoire.';
        if (result === 'lost') msg = 'Dommage ! Essayez encore.';
        if (result === 'duel_result') {
            if (!s.duelState) return;
            const p1 = s.duelState.p1;
            const p2 = s.duelState.p2;

            if (p1.score > p2.score) msg = "Victoire du Joueur 1 (Score) !";
            else if (p2.score > p1.score) msg = "Victoire du Joueur 2 (Score) !";
            else {
                // Tie on score, check time (Lower is better)
                if (p1.time < p2.time) msg = "Égalité aux points... Joueur 1 gagne au temps !";
                else if (p2.time < p1.time) msg = "Égalité aux points... Joueur 2 gagne au temps !";
                else {
                    if (p1.lives > p2.lives) msg = "Égalité totale... Joueur 1 gagne aux vies !";
                    else if (p2.lives > p1.lives) msg = "Égalité totale... Joueur 2 gagne aux vies !";
                    else msg = "Égalité parfaite ! Incroyable.";
                }
            }
        }
