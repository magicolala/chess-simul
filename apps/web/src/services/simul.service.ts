...
export class SimulService {
  ...
  joinSimul(simulId: string) {
    ...
    updatedSimul.challengers = [...updatedSimul.challengers, {
      id: user.id,
      name: user.name,
      avatar: user.avatar,
      elo: 1200,
  ...
  private createMockSimul(hostName: string, elo: number, time: number, inc: number, status: 'open' | 'started', max: number): SimulEvent {
    return {
      id: Math.random().toString(36).substring(7).toUpperCase(),
      host: { name: hostName, avatar: `https://api.dicebear.com/7.x/notionists/svg?seed=${hostName}`, elo },
      config: { timeMinutes: time, incrementSeconds: inc, opponentCount: max, difficulty: 'pvp' },
      status,
      challengers: Array(Math.floor(Math.random() * max)).fill(null).map((_, i) => ({
          id: `p${i}`,
          name: `Challenger_${i}`,
