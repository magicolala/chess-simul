import { Component, inject, signal, output, computed, type OnInit } from '@angular/core';
...
@Component({
  ...
  template: `
    <div
      class="max-w-6xl mx-auto p-4 md:p-8 font-sans h-[calc(100vh-100px)] flex flex-col md:flex-row gap-6"
    >
      ...
      <div class="w-full md:w-1/3 flex flex-col gap-6">
                ...
                <div class="ui-card p-2 flex items-center justify-between">
                  ...
                  <div class="flex space-x-1">
                    <button
                      ...
                      class="ui-btn ui-btn-dark px-2 py-1 text-[10px]"
                    >
                      Oui
                    </button>
                    <button
        ...
        <div class="ui-card flex-1 flex flex-col overflow-hidden">
          <div class="ui-card-header p-4 flex justify-between items-center">
            ...
            <button
              (click)="showAddInput.set(!showAddInput())"
              class="ui-btn ui-btn-dark text-xs px-2 py-1"
            >
              + Ajouter
            </button>
          </div>
          ...
          @if (showAddInput()) {
            <div class="ui-card-footer p-2 flex gap-2 bg-gray-100 dark:bg-gray-800">
              ...
              <button
                (click)="addFriend(addInput.value); addInput.value = ''"
                class="ui-btn ui-btn-dark px-3 text-xs"
              >
                OK
              </button>
            </div>
              ...
              >
                <div class="flex items-center space-x-3">
                  <div class="relative">
                    <img
                      [src]="friend.avatar"
                      class="w-10 h-10 border border-current rounded-full bg-gray-200"
                    />
                    <div
                      class="absolute -bottom-1 -right-1 w-3 h-3 border border-white rounded-full"
                      [class.bg-green-500]="friend.status === 'online'"
                      [class.bg-yellow-500]="friend.status === 'ingame'"
                      [class.bg-gray-400]="friend.status === 'offline'"
                    ></div>
                  </div>
                  <div>
                ...
                <div class="opacity-0 group-hover:opacity-100 transition-opacity flex space-x-2">
                  <button
                    (click)="$event.stopPropagation(); goToProfile.emit(friend.id)"
                  ...
                  <button
                    (click)="$event.stopPropagation(); challenge(friend.id)"
                    class="text-xs hover:scale-110 transition-transform"
                    title="Défier"
                  >
                    ⚔️
        ...
        @if (selectedFriend(); as friend) {
          <div class="ui-card-header p-4 flex justify-between items-center">
            <div class="flex items-center space-x-3">
              <img
                [src]="friend.avatar"
                class="w-10 h-10 border-2 border-[#1D1C1C] dark:border-white rounded-full bg-white"
              />
              <div>
                <h3 class="font-black text-lg uppercase text-[#1D1C1C] dark:text-white">
                  {{ friend.name }}
                </h3>
            ...
            <div class="flex space-x-2">
              <button
                (click)="challenge(friend.id)"
                class="ui-btn ui-btn-secondary px-4 py-2 text-xs font-black"
              >
                Défier
              </button>
            </div>
          ...
          <div class="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-100 dark:bg-[#0f0f0f]">
            @for (msg of currentMessages(); track msg.timestamp) {
              <div
                class="flex flex-col"
                [class.items-end]="msg.senderId !== friend.id"
                [class.items-start]="msg.senderId === friend.id"
              >
                <div
                  class="max-w-[70%] p-3 border-2 border-[#1D1C1C] text-sm font-medium"
