            ...
            <div class="flex-1 space-y-8">
                ...
                <div class="bg-white dark:bg-[#1a1a1a] border-2 border-[#1D1C1C] dark:border-white p-6 wero-shadow">
                    ...
                    <div class="flex items-center space-x-4 mb-4">
                        <div class="flex items-center space-x-2 bg-gray-100 dark:bg-gray-800 px-3 py-1 rounded-[2px] border border-gray-300 dark:border-gray-600">
                             <span class="text-xl">⏱</span>
                             <span class="font-mono font-bold">{{ room.config.timeMinutes }}+{{ room.config.incrementSeconds }}</span>
                        </div>
                        <div class="flex items-center space-x-2 bg-gray-100 dark:bg-gray-800 px-3 py-1 rounded-[2px] border border-gray-300 dark:border-gray-600">
                             <span class="text-xl">🔒</span>
                             <span class="font-bold text-sm uppercase">{{ room.isPrivate ? 'Privé' : 'Public' }}</span>
                        </div>
                    </div>

                    @if (room.isPrivate) {
                        <div class="flex items-center space-x-2 bg-[#FFF48D] p-2 border border-[#1D1C1C] text-sm">
                            <span class="font-bold">Code:</span>
                            <code class="font-mono font-bold text-lg select-all">{{ room.id }}</code>
                            <span class="text-[10px] text-gray-500 ml-auto uppercase font-bold">Copier pour inviter</span>
                        </div>
                    }
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    @for (player of room.players; track player.id) {
                        <div class="bg-white dark:bg-[#1a1a1a] border-2 p-6 flex flex-col items-center justify-center relative wero-shadow transition-all"
                            ...
                            @if (player.isReady) {
                                <div class="absolute top-2 right-2 bg-green-500 text-white text-[10px] font-black uppercase px-2 py-0.5">Prêt</div>
                            } @else {
                                <div class="absolute top-2 right-2 bg-gray-200 text-gray-500 text-[10px] font-black uppercase px-2 py-0.5">Attente</div>
                            }

                            <div class="w-20 h-20 rounded-full border-4 border-[#1D1C1C] dark:border-white overflow-hidden mb-3 bg-gray-100">
                                <img [src]="player.avatar" class="w-full h-full object-cover">
                            </div>
                            <h3 class="text-lg font-black font-display uppercase truncate max-w-full">{{ player.name }}</h3>
                            <div class="mt-2 flex space-x-2">
                                <span class="w-4 h-4 border border-black" [class.bg-white]="player.side === 'w'" [class.bg-black]="player.side === 'b'"></span>
                                <span class="text-xs font-bold text-gray-500 uppercase">{{ player.side === 'w' ? 'Blancs' : 'Noirs' }}</span>
                            </div>
                        </div>
                    }
                    ...
                    @if (room.players.length < 2) {
                        <div class="border-2 border-dashed border-gray-300 dark:border-gray-700 p-6 flex flex-col items-center justify-center text-gray-400">
                ...
                <div class="flex-1 p-4 space-y-3 overflow-y-auto min-h-[300px]">
                     @for (msg of room.messages; track $index) {
                         <div class="flex flex-col items-start">
                             <div class="bg-gray-100 dark:bg-gray-800 p-2 rounded-[2px] text-xs font-medium border border-gray-200 dark:border-gray-700">
                                 <span class="font-bold text-[#1D1C1C] dark:text-gray-300 mr-1">{{ msg.sender }}:</span>
