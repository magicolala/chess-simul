...
@Component({
  ...
  template: `
    @if (game()) {
      <div
        class="h-full flex flex-col md:flex-row max-w-7xl mx-auto md:p-4 gap-4 font-sans relative"
      >
        <!-- Connection Status Banner -->
        @if (
          mpService.connectionStatus() === 'poor' || mpService.connectionStatus() === 'offline'
        ) {
          <div
            class="absolute top-0 left-0 w-full z-30 bg-red-500 text-white text-xs font-bold uppercase text-center py-1 animate-pulse"
          >
        ...
        <div class="flex-1 flex flex-col">
          ...
          <div class="ui-card p-2 mb-2 flex justify-between items-center">
            <div class="flex items-center space-x-3">
              <img
                ...
                class="w-10 h-10 border border-[#1D1C1C] dark:border-white rounded-full"
              />
              <div>
                <div class="flex items-center space-x-2">
                  ...
                  <div
                    ...
                    [class.bg-red-500]="
                      mpService.connectionStatus() === 'poor' ||
                      mpService.connectionStatus() === 'offline'
                    "
                  ></div>
                </div>
            ...
            <div
              class="font-mono text-xl font-black px-4 py-1 bg-gray-100 dark:bg-gray-800 rounded-[2px]"
          ...
          <!-- BOARD -->
          <div
            class="flex-1 bg-[#e0e0e0] dark:bg-gray-900 border-2 border-[#1D1C1C] dark:border-white flex items-center justify-center p-2 relative"
          >
            ...
            @if (game()!.status !== 'active' && game()!.status !== 'waiting') {
              <div
                class="absolute inset-0 bg-[#1D1C1C]/80 z-20 flex flex-col items-center justify-center backdrop-blur-sm animate-in fade-in zoom-in"
              >
                ...
                <div class="flex items-center space-x-2 text-2xl font-black mb-8">
                  ...
                  <span
                    [class.text-green-400]="(game()!.eloChange || 0) > 0"
                ...
                <div class="flex space-x-4">
                  <button
                    (click)="leave()"
                    class="px-6 py-3 bg-white text-[#1D1C1C] font-black uppercase border-2 border-white hover:bg-gray-200"
                  >
                    Menu Principal
                  </button>
                  <button
                    class="px-6 py-3 bg-[#7AF7F7] text-[#1D1C1C] font-black uppercase border-2 border-[#1D1C1C] wero-shadow hover:bg-[#FFF48D]"
                  >
                    Revanche
                  </button>
                </div>
          ...
          <div class="ui-card p-2 mt-2 flex justify-between items-center">
            <div class="flex items-center space-x-3">
              <img
                [src]="game()!.playerName === 'Joueur 1' ? auth.currentUser()?.avatar : '...'"
                class="w-10 h-10 border border-[#1D1C1C] dark:border-white rounded-full"
              />
              <div>
                <p class="font-bold text-sm text-[#1D1C1C] dark:text-white leading-none">
        ...
        <div class="ui-card w-full md:w-80 flex flex-col">
          <!-- Chat History -->
          <div
            class="flex-1 bg-gray-50 dark:bg-[#121212] p-4 overflow-y-auto space-y-2 min-h-[200px]"
          >
            <div class="text-center text-[10px] font-bold text-gray-400 uppercase mb-4">
              Début de partie
            </div>
            @for (msg of game()!.chat; track msg.id) {
              <div
                class="flex flex-col"
                [class.items-end]="msg.isSelf"
                [class.items-start]="!msg.isSelf"
              >
                <div
                  class="max-w-[85%] p-2 border-2 border-[#1D1C1C] text-xs font-medium"
          ...
          <div class="ui-card-footer p-2">
            <div class="flex space-x-2">
              ...
              />
              <button (click)="sendChat()" class="ui-btn ui-btn-dark px-3 text-xs">></button>
            </div>
          </div>
          ...
          <!-- Report/Block (Bottom) -->
          <div
            class="p-2 flex justify-between bg-white dark:bg-[#1a1a1a] border-t-2 border-[#1D1C1C] dark:border-gray-800"
          >
            <button
              (click)="report()"
              class="text-[10px] text-gray-400 hover:text-red-500 uppercase font-bold"
            >
              Signaler
            </button>
            <button
