...
export class MemoryModePageComponent {
  ...
  constructor() {
    // We let the user choose the mode now, so no auto-start.
    // Ensure we are in start state or reset
    if (this.state().status !== 'start' && this.state().status !== 'playing') {
      // If coming back, maybe reset?
    }
  }
  ...
  resetGame() {
    // "Menu" button basically resets to start screen
    // We can do this by setting state to start or just reloading.
    // Ideally service has a method to go to menu.
    // For now, let's just make it clear.
    // Actually `reset()` in service does `startGame`.
    // We want a `goToMenu` or manual reset.
    // Let's modify service state directly or add method.
    // For now, I'll update the `state` via a new method in service or just assume resetGame restarts the CURRENT mode?
    // The UI says "Menu" in header. "Rejouer" in result.
    // "Rejouer" -> Restart same mode.
    // "Menu" -> Go to start screen.

    // I'll update service to handle this logic cleanly in next step if needed, 
    // but for now let's implement a clean `resetToMenu` in service or handle here.
    // I'll inject `reset()` which currently does `startGame(currentMode)`.
    // I need a way to go back to selection.
    // I will call `memoryService.state.set(...)` locally? No, clean service call.
    // I'll add `resetToMenu` to service in next step? 
    // Or I can just manually set it here since it's a writable signal exposed? 
    // `state` is a signal, I can update it.
