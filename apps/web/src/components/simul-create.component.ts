...
import { type GameConfig } from '../services/chess-logic.service';
  ...
  template: `
    <div class="max-w-4xl mx-auto p-8 font-sans animate-in fade-in zoom-in duration-300">
      <div class="ui-card p-8">
        <div
          class="flex items-center space-x-4 mb-8 pb-8 border-b-2 border-[#1D1C1C] dark:border-gray-700"
        >
          <div
            class="w-16 h-16 bg-[#FFF48D] flex items-center justify-center text-4xl border-2 border-[#1D1C1C]"
          >
            ♟
          </div>
          <div>
        ...
        <div class="space-y-8">
          ...
          <div class="grid grid-cols-2 gap-6">
            <div>
              ...
              <div class="flex space-x-2">
                <button
                  (click)="timeMinutes.set(10)"
                  [class.bg-[#1D1C1C]]="timeMinutes() === 10"
                  [class.text-white]="timeMinutes() === 10"
                  class="ui-btn ui-btn-ghost flex-1 py-2 font-bold"
                >
                  10
                </button>
                <button
                  (click)="timeMinutes.set(20)"
                  [class.bg-[#1D1C1C]]="timeMinutes() === 20"
                  [class.text-white]="timeMinutes() === 20"
                  class="ui-btn ui-btn-ghost flex-1 py-2 font-bold"
                >
                  20
                </button>
                <button
                  (click)="timeMinutes.set(40)"
                  [class.bg-[#1D1C1C]]="timeMinutes() === 40"
                  [class.text-white]="timeMinutes() === 40"
                  class="ui-btn ui-btn-ghost flex-1 py-2 font-bold"
                >
                  40
                </button>
          ...
          <div
            class="ui-card-header p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4"
          >
            <div>
              <p class="text-sm font-black uppercase text-[#1D1C1C]">Preset Hydra</p>
              <p class="text-xs font-bold text-gray-700">
                Cadence recommandée 5 min + 3s, scoring +3 / +1 / -1.
              </p>
            </div>
            <button (click)="useHydraPreset()" class="ui-btn ui-btn-dark px-4 py-2">
              Appliquer 5+3
            </button>
          </div>
          ...
          <div class="pt-6 border-t-2 border-[#1D1C1C] dark:border-gray-700">
            <button
              (click)="launchSimul()"
              class="ui-btn ui-btn-secondary w-full py-4 text-xl font-black font-display"
            >
              Créer la Simultanée
            </button>
          </div>
