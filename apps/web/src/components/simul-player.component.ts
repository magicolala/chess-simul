      ...
      <div class="h-full flex flex-col items-center justify-center p-4 font-sans max-w-5xl mx-auto">
        ...
        <div class="ui-card bg-[#1D1C1C] text-white p-4 mb-4 flex justify-between items-center">
          <div class="flex items-center space-x-4">
            <img
              [src]="simulService.currentSimul()?.host?.avatar"
              class="w-12 h-12 rounded-full border-2 border-white bg-white"
            />
            <div>
              <h2 class="text-xl font-black font-display uppercase leading-none">
                Simultanée vs {{ simulService.currentSimul()?.host?.name }}
              </h2>
        ...
        <div class="ui-card p-6 w-full gap-8 flex flex-col md:flex-row relative">
          ...
          @if (game()!.turn === 'w' && game()!.status === 'active') {
            <div
              class="absolute inset-0 z-20 bg-white/50 dark:bg-black/50 backdrop-blur-[2px] flex flex-col items-center justify-center pointer-events-none"
            >
              <div
                class="ui-card bg-[#1D1C1C] text-white px-6 py-3 border-2 border-white animate-pulse"
              >
          ...
          <div class="w-full md:w-64 flex flex-col space-y-4">
            ...
            <div class="ui-card flex-1 flex flex-col h-64">
              <div class="ui-card-header bg-[#1D1C1C] text-white text-xs font-bold uppercase p-2">
                Chat avec l'hôte
              </div>
              ...
              <input
                ...
                placeholder="Message..."
                class="ui-input-top text-xs"
              />
            </div>

            <button (click)="resign()" class="ui-btn ui-btn-ghost w-full py-3 text-sm text-red-600">
              Abandonner
            </button>
            <button
              (click)="quit.emit()"
              class="ui-btn ui-btn-ghost w-full py-3 text-sm text-gray-500"
            >
              Quitter
            </button>
          </div>
