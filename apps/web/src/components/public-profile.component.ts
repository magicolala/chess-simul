...
@Component({
  ...
  template: `
    @if (viewedProfile(); as profile) {
      <div
        class="max-w-6xl mx-auto p-4 md:p-8 font-sans animate-in fade-in slide-in-from-bottom-4 duration-500"
      >
        <!-- Header Card -->
        <div class="ui-card p-8 mb-8 relative overflow-hidden">
          <!-- Background Decoration -->
          <div
            class="absolute top-0 right-0 p-8 opacity-5 text-9xl font-display pointer-events-none"
          >
            ♟
          </div>
          ...
          <div class="flex flex-col md:flex-row items-center md:items-start gap-8 relative z-10">
            <div class="relative group">
              <img
                [src]="profile.avatar"
                class="w-32 h-32 rounded-full border-4 border-[#1D1C1C] dark:border-white bg-gray-100 object-cover"
              />
              @if (isMe(profile.id)) {
                <div
            ...
            <div class="flex-1 text-center md:text-left">
              <h2
                class="text-4xl font-black font-display uppercase text-[#1D1C1C] dark:text-white mb-2"
              >
                {{ profile.name }}
              </h2>
              <p class="text-gray-600 dark:text-gray-300 font-medium mb-4 max-w-lg">
                {{ profile.bio }}
              </p>

              <div class="flex flex-wrap justify-center md:justify-start gap-3">
                <span class="ui-chip text-gray-600 dark:text-gray-300 bg-gray-100 dark:bg-gray-800">
                  Membre depuis {{ profile.joinedAt | date: 'mediumDate' }}
                </span>
                @if (!isMe(profile.id)) {
                  <button class="ui-btn ui-btn-dark px-4 py-1 text-xs">Ajouter en ami</button>
                  <button class="ui-btn ui-btn-ghost px-4 py-1 text-xs">Message</button>
                }
              </div>
            </div>

            <!-- Highlight Stat -->
            <div class="ui-card-header p-4 text-center w-full md:w-auto">
              <p class="text-xs font-black uppercase text-[#1D1C1C]/70 mb-1">Meilleur ELO</p>
              <p class="text-4xl font-black font-display text-[#1D1C1C]">
                {{ getMaxElo(profile) }}
              </p>
            </div>
          </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div class="lg:col-span-2 space-y-8">
            ...
            <div>
              <h3
                class="text-xl font-black font-display uppercase mb-4 text-[#1D1C1C] dark:text-white border-b-2 border-[#1D1C1C] dark:border-white pb-2 inline-block"
              >
                Classement
              </h3>
              <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div
                  class="ui-card p-4 wero-shadow-sm hover:translate-y-[-2px] transition-transform"
                >
            ...
            <div>
              ...
              <div class="flex flex-wrap gap-4">
                @for (badge of profile.badges; track badge.id) {
                  <div
                    class="ui-card p-3 flex items-center space-x-3 pr-6 rounded-[2px] group relative cursor-help bg-gray-50 dark:bg-[#121212]"
                  >
                    ...
                    <div>
                      ...
                      <p class="text-[10px] text-gray-500 mt-1">
                        {{ badge.unlockedAt | date: 'shortDate' }}
                      </p>
                    </div>

                    <!-- Tooltip -->
                    <div
                      class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 bg-[#1D1C1C] text-white text-xs p-2 opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-20 text-center border-2 border-[#1D1C1C]"
                    >
                      {{ badge.description }}
                      <div
                        class="absolute top-full left-1/2 -translate-x-1/2 border-4 border-transparent border-t-[#1D1C1C]"
                      ></div>
                    </div>
                  </div>
                }
          ...
          <div class="ui-card p-6 h-fit">
            ...
            <div class="space-y-4">
              <div class="flex items-center space-x-3 border-b border-gray-100 pb-3">
                ...
                <p class="text-xs">
                  A gagné contre <span class="font-bold">MagnusCarlsenClone</span> en Blitz.
                </p>
              </div>
              <div class="flex items-center space-x-3 border-b border-gray-100 pb-3">
              ...
              <div class="flex items-center space-x-3">
                ...
                <p class="text-xs">
                  A rejoint le tournoi <span class="font-bold italic">Sunday Bullet Arena</span>.
                </p>
              </div>
            </div>
          </div>
